...
> 已知函数$f(x)$在$\left[ a, b \right]$上连续，在$(a, b)$内可导，且$f'(x) = 0$仅在有限多个点处成立

在$(a, b)$内任取两点$x_1 \lt x_2$，则$f(x)$在$\left[ a, b \right]$上连续、在$(a, b)$内可导，则根据拉格朗日中值定理可知：
$$
\exists \xi \in (x_1, x_2), f(x_2) - f(x_1) = f'(\xi) (x_2 - x_1)
$$

- 当在$(a, b)$内$f'(x) \geq 0$时：
    
    由上述的拉格朗日中值定理的结论可知：
    $$
    f'(\xi) (x_2 - x_1) \geq 0 \Rightarrow f(x_2) \geq f(x_1)
    $$
    若$f(x_1) = f(x_2)$，又因为：
    $$
    ...
    则：
    $$
    \forall x \in (x_1, x_2), f(x) \equiv f(x_1) = f(x_2) = C
    $$
    即在此区间内函数值恒为一个常数，则此区间内的导数皆为零，便存在有无穷多个导数为零的点，这与前提条件相悖，所以$f(x_1) = f(x_2)$不成立，说明函数$f(x)$在区间$\left[ a, b \right]$上严格单调递增
    
- 当在$(a, b)$内$f'(x) \leq 0$时：
    
    由上述的拉格朗日中值定理的结论可知：
    $$
    f'(\xi) (x_2 - x_1) \leq 0 \Rightarrow f(x_2) \leq f(x_1)
    $$
    若$f(x_1) = f(x_2)$，又因为：
    $$
    \forall x \in (x_1, x_2), f(x_1) \geq f(x) \geq f(x_2)
    $$
    则：
    $$
    \forall x \in (x_1, x_2), f(x) \equiv f(x_1) = f(x_2) = C
    $$
    即在此区间内函数值恒为一个常数，则此区间内的导数皆为零，便存在有无穷多个导数为零的点，这与前提条件相悖，所以$f(x_1) = f(x_2)$不成立，说明函数$f(x)$在区间$\left[ a, b \right]$上严格单调递减
